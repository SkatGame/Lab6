// Copyright 2021 Skat xaxaxaxa232@mail.ru

#include <header.hpp>

Hasher::Hasher(int argc, char **argv) {
  if (argc == 1) {
    number_threads = std::thread::hardware_concurrency(); // количество потоков максимальное
  } else {
    number_threads = static_cast<unsigned int>(std::stoi(argv[1])); //введёное
  }
  duration = 0;
}

void Hasher::start(const bool& key) {
  logging::add_common_attributes(); //Подключает обычные атрибуты для логирования
  logging::add_console_log(std::clog, keywords::format = "[%Severity%] %TimeStamp%: %Message%"); //Добовляет консоль логирования

  logging::add_file_log //Логирование файла
      (
